...
export default class DelayLineExample extends React.Component {
  constructor(props) {
    ...
    this.onContextInit = (ctx) => {
      ...
      setInterval(
        () =>
          this.setState({ start: ctx.currentTime, stop: ctx.currentTime + 3 }),
        6000
      );
    };
  }
    ...
    return (
      <RAudioContext debug={true} onInit={this.onContextInit}>
        <article>
          ...
          <p>
            This example demonstrates how one can create feedback delay lines
            using the <code>RCycle</code> component. It also shows how
            scheduling works.
          </p>
          <p>
            Make sure to always include a <code>RGain</code> with{" "}
            <code>gain</code> &lt; 1 to avoid infinite feedback.
          </p>
        </article>
        <RPipeline>
          <ROscillator
